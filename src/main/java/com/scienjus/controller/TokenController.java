...
public class TokenController {
    ...
    public ResponseEntity<ResultModel> logout(@CurrentUser User user) {
        System.out.println(tokenManager.getToken("2f79d0caba7c481592d803da31373275"));
    ...
    public ResponseEntity<ResultModel> findToken(@RequestParam String token) {
        TokenModel tokenModel= tokenManager.getToken(token);
        System.out.println(tokenManager.checkToken(tokenModel));
        //tokenManager.deleteToken(user.getId());
        return new ResponseEntity<>(ResultModel.ok(), HttpStatus.OK);
    }
